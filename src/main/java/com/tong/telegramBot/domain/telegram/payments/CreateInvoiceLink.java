...
public class CreateInvoiceLink {
    ...
    private Integer maxTipAmount; // The maximum accepted amount for tips in the smallest units of the currency (integer, not float/double). For example, for a maximum tip of US$ 1.45 pass max_tip_amount = 145. See the exp parameter in currencies.json, it shows the number of digits past the decimal point for each currency (2 for the majority of currencies). Defaults to 0
    ...
    private String photoUrl; // URL of the product photo for the invoice. Can be a photo of the goods or a marketing image for a service.
    ...
    private Boolean sendPhoneNumberToProvider; // Pass True if the user's phone number should be sent to the provider
    @JsonProperty("send_email_to_provider")
    private Boolean sendEmailToProvider; // Pass True if the user's email address should be sent to the provider
